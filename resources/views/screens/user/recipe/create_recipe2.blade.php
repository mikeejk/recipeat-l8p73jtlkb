...
<body class="antialiased bg-black ">
    ...
    <main class="mx-auto w-7/12  py-10 mt-10 justify-center item-center">
        <p class="text-lg text-gray-100 ">Step 2 <span class="text-Montserrat opacity-50">of 4</span></p>
        <div class="h-0.5 mt-4 w-11/12 bg-gray-500 rounded-full">
            <div class="w-2/4 h-0.5 bg-white"></div>
        </div>
        <h1 class="text-white py-4">Ingredients</h1>
        <section class="bg-black">
            <main class="mx-auto w-11/12 bg-black py-2 justify-start">
                <div class="w-full py-4">
                    <input type="text"
                        class="py-4 border-b-2 bg-transparent w-full text-start justify-satrt placeholder:text-2xl text-white placeholder:text-start"
                        placeholder="Enter the title">
                    {{-- <h1 class="py-4 border-b-2 bg-transparent text-2xl text-white w-full text-start justify-satrt">Enter the title</h1> --}}
                </div>
                <div class="form-group row justify-between">
                    <!-- Form-Next Row::start -->
                    <table id="myTable" class=" table space-x-8 order-list">
                        <thead>
                            <tr>
                                <th class="text-gray-400 item-center">Ingredient</th>
                                <th class="text-gray-400 item-center">Quantity</th>
                                <th class="text-gray-400 item-center">Units</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr>
                                <td>
                                    <input type="text"
                                        class="py-2  rounded-lg bg-gray-500 w-full text-start justify-start"
                                        style="color:#292D32;" placeholder="Eg:Milk" />
                                    </td>
                                <td><input type="text"
                                        class="py-2  rounded-lg bg-gray-500  w-full text-start justify-start"
                                        style="color:#292D32;"placeholder="Eg:200" /></td>
                                <td><select
                                        class="py-2 rounded-lg bg-gray-500 w-20 text-white text-start justify-start"
                                        style="color:#292D32;">
                                        @foreach ($measurements as $measurement)
                                            <option>{{ $measurement->measurement }}</option>
                                        @endforeach
                                    </select>
                                </td>
                                <td>
                                    <button type="button"id="addrow"
                                        class="py-1 bg-white text-black rounded-lg w-20">Add</button></td>
                            </tr>
                        </tbody>
                    </table>
                    <!-- Form-Next Row::end -->
                </div>
                <div class="mt-20"></div><button type="button"
                    class="py-2 rounded-lg  w-50 bg-white text-black text-start item-center justify-start">Add next set
                    of Ingredients</button></div>
                {{-- <button class="bg-gray-500   text-white rounded lg:text-lg text-base focus:outline-none">Save to
                    Drafts</button>
                <div class="w-full  items-center justify-end py-5 space-x-2">
                    <a href="#" class="bg-gray-500 mx-auto  text-white rounded lg:text-lg text-base focus:outline-none"><i
                            class="fas fa-angle-left text-white mt-1 mr-1"></i>Previous</button>
                    <button
                        class="bg-red-600 text-white w-28 py-2 flex justify-center item-center rounded-md">Next,Steps<i
                            class="fas fa-angle-right text-white mt-1 ml-1"></i></button>
                </div> --}}
            </main>
        </section>

    </main>
    <div class="w-full flex px-2 py-2  justify-between">
...
<script>
        $(document).ready(function() {
            ...
            $("#addrow").on("click", function() {
                ...
                cols +=
                '<td><input type="text"class="py-2  rounded-lg bg-gray-500 w-full text-start justify-start"style="color:#292D32;" placeholder="Eg:Milk"name="quantity[]' + counter + '" /></td>'
                   
                cols +=
                    '<td><input type="text" class="py-2  rounded-lg bg-gray-500 w-full text-start justify-start"style="color:#292D32;" placeholder="Eg:Milk"name="quantity[]' + counter + '"form-control @error('quantity') is-invalid @enderror"/></td>';

                 cols +=
                ' <td><select class="py-2 rounded-lg bg-gray-500 w-20 text-white text-start justify-start"style="color:#292D32;name="measurement[]' + counter +'"> @foreach ($measurements as $measurement)<option value="{{$measurement->id}}">{{ $measurement->measurement }}</option>@endforeach</select></td>';
                   
                cols +=
                    '<td><input type="button" class="ibtnDel py-1 bg-white text-black rounded-lg w-20" value="Delete"></td>';
                newRow.append(cols);
